...
def load_cifar10(transform: Callable = transforms.ToTensor(), batch_size: int = 256) -> tuple[TensorLoader, TensorLoader]:
    ...
    dl_train = TensorLoader.from_dataset(ds_train, size=10000, batch_size=batch_size, shuffle=True, loader_workers=0)
    dl_eval = TensorLoader.from_dataset(ds_eval, size=5000, batch_size=batch_size, shuffle=False, loader_workers=0)
...
def load_imagenet(transform: Callable, batch_size: int = 256) -> tuple[TensorLoader, TensorLoader]:
    ...
    ds_eval = PartialImageFolder(root="/datasets/ImageNet/val", size=5000, allow_empty=False, transform=transform)
