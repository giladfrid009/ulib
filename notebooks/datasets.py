...
def load_imagenet(transform: Callable, batch_size: int = 256) -> tuple[TensorLoader, TensorLoader]:
    ...
    dl_train = TensorLoader.from_dataset(
        ds_train, batch_size=batch_size, shuffle=True, loader_batch_size=1024, loader_workers=5
    )
    dl_eval = TensorLoader.from_dataset(
        ds_eval, batch_size=batch_size, shuffle=False, loader_batch_size=1024, loader_workers=5
    )
    return dl_train, dl_eval
